...
class AnthropicWebSearch:
    ...
    Produces a provider-native ToolSpec that instructs the Anthropic adapter to
    include the `web_search_20250305` server tool definition. This is not a
    local tool and will not be executed in the agent; Claude runs it on server.

    Args:
        allowed_domains: List of domains to allow searching (cannot be used with blocked_domains)
        blocked_domains: List of domains to block searching (cannot be used with allowed_domains)
        user_location: User location for more relevant search results
