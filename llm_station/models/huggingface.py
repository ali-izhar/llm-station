...
class HuggingFaceProvider(ProviderAdapter):
    ...
    def __init__(self, api_key: Optional[str] = None, endpoint_url: Optional[str] = None, serpapi_key: Optional[str] = None, **kwargs: Any) -> None:
        ...
        if not self.api_key:
            raise ValueError("HuggingFace API key not provided. Set api_key parameter or HUGGING_FACE_API_TOKEN environment variable")
        if not self.endpoint_url:
            raise ValueError("HuggingFace endpoint URL not provided. Set endpoint_url parameter or HUGGING_FACE_ENDPOINT_URL environment variable")
    ...
    def _perform_web_search(self, query: str, num_results: int = 5) -> List[Dict[str, Any]]:
        """Perform web search using SerpAPI."""
        if not self.serpapi_key:
            raise ValueError("SerpAPI key not configured. Set serpapi_key or SERPAPI_API_KEY environment variable")
        ...
        try:
            ...
            raise ImportError("SerpAPI package not installed. Install with: pip install google-search-results")
    ...
    def _extract_text_from_url(self, url: str, max_length: int = 5000) -> str:
        ...
        try:
            ...
            # Final fallback
            return response.text[:max_length] if len(response.text) > max_length else response.text
    ...
    def _format_search_context(
        ...
        if extract_full_text:
            ...
            urls = [result.get('link', '') for result in search_results[:max_urls] if result.get('link')]
            extracted_texts = []
            for url in urls:
    ...
    def _generate_with_web_search(
        ...
        use_web_search = hf_config.use_web_search if hf_config.use_web_search is not None else True
        ...
        if use_web_search and user_query:
            try:
                search_results = self._perform_web_search(
                    user_query, 
                    num_results=hf_config.num_search_results or 5
                )
        ...
        # Make API call to HuggingFace endpoint
    ...
    def generate(
        ...
        if tools:
            for tool in tools:
                if tool.provider == "huggingface" and tool.provider_type == "web_search":
                    has_web_search = True
                    # Override web search config from tool config
                    if tool.provider_config:
                        if "use_web_search" in tool.provider_config:
                            hf_config.use_web_search = tool.provider_config["use_web_search"]
                        if "extract_full_text" in tool.provider_config:
                            hf_config.extract_full_text = tool.provider_config["extract_full_text"]
                        ...
                        if "num_search_results" in tool.provider_config:
                            hf_config.num_search_results = tool.provider_config["num_search_results"]
                    break
